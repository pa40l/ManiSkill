...
set -euo pipefail
rm -rf logs/*
NO_VIDEO=1 bash skills/maniskill-remote-planner-analysis/scripts/remote_analyze.sh myrobocasa_takeitback_planner 100 25 >/tmp/takeitback-autoresearch.out 2>&1
uv run python - <<'PY'
import glob, json
success = 0
failures = {}
