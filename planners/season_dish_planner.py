...
#: The stock Fetch's limit on a roll joint (rad). The pour picks its direction with this
#: in mind — see `wrist_roll_for_tilt`. Since the URDF went back to +-3.141 the candidate
#: scan cannot leave the band anyway, so the preference is satisfied trivially and the
#: shorter-turn rule decides alone; it still guards a tree that widens the URDF, where it
#: was worth 8 recordings of 200. 0 = off (the pre-2026-09-10 behaviour).
STOCK_ROLL_LIMIT = float(os.environ.get("MIKASA_STOCK_ROLL_LIMIT", "3.141"))
...
def wrist_roll_for_tilt(task, T_tcp_obj: sapien.Pose, tilt_deg: float, *, step_deg: float = 2.0,
    ...
    for sign in signs:
        for k in range(1, int(max_turn_deg / step_deg) + 1):
            ...
            if tilt >= tilt_deg:
                found.append((float(qw), tilt, sign, k))
                break
    if not found:
        return None
    # Both directions tip the condiment; only one of them may leave the wrist where an
    # unmodified Fetch could hold it. This mattered while our URDF gave the roll joints
    # +-6.28 (mplib refuses a `continuous` joint) against the stock +-3.141; the pour is the
    # last thing that happens in the episode — 2026-09-10, seed 3811: a wrist parked at
    # +1.25 after the grasp poured at +4.11, past the stock limit, and the whole tail of
    # the recording became unreplayable outside this fork. Turning the other way reaches
    # the same tilt at -2.17. Preference only: when no direction is stock-safe, or when
    # STOCK_ROLL_LIMIT is 0, the old shorter-turn rule decides alone.
    if STOCK_ROLL_LIMIT > 0:
        safe = [c for c in found if abs(c[0]) <= STOCK_ROLL_LIMIT]
        if safe:
            found = safe
    best = None
    for cand in found:                                          # `found` is in `signs` order
