...
def side_grasp_pose(task, depth: float, lift: float = 0.0):
...
def ready_by_line(env, planner, task, ready, *, drive: float, grasp, label: str = "ready posture"):
    ...
    and the grasp screw to `grasp` from where the drive ends. The first line whose
    ...
    for k, g in enumerate(order[:READY_IK_GOALS]):
        ...
        if sweep.get("status") != "Success":
            ...
            continue
        deeper = mplib.Pose(p=np.asarray(grasp.p, dtype=np.float64) + [0.0, READY_GRASP_SLACK, 0.0],
                            q=grasp.q)
        try:
            # the cup out of the world for this check: the deeper target would put the
            # palm inside it; what is checked here is the arm against the CABINET
            with common.touchable(planner, "cup"):
