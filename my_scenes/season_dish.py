...
class SeasonDishTask(BaseEnv):
    ...
    def _default_sensor_configs(self):
        """None — the observation carries the ROBOT's cameras and nothing else.

        Until 2026-09-10 this task added a `base_camera`: a fixed 128 px wide-angle
        standing off the counter, framed by `_view_point`. It was inherited from the
        template and it is a camera no Fetch has — a third view, from the room, that a
        policy would learn to rely on and a real robot could never provide. Under
        `obs_mode="rgb"` it appeared in `obs["sensor_data"]` next to `fetch_head` and
        `fetch_hand` (a recorded episode carried `base_camera/rgb (495, 128, 128, 3)`);
        it was never in the dataset (`tools/vla/h5_to_lerobot.py` reads exactly the
        robot's two), but it cost a render pass and about 22 KB a step of h5. Removed on
        the owner's word, 2026-09-10 — the same removal was made on `fix/same-drawer`
        ("удали камеру base для человека, оставь только две которые у робота") and on
        `feat/task4` («камеру не пишем — только те, что на роботе сидят»).
        `MikasaDepthRecall-v1`, `MikasaCabinetSearch-v0` and the retrieval family
        already return nothing here.

        The HUMAN render camera below is untouched: it is the clip's viewpoint
        (`_default_human_render_camera_configs`), rendering rather than observation —
        it never enters `sensor_data` or `obs`, and no threshold reads it.
        """
        return []
