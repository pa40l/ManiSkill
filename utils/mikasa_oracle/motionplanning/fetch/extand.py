...
class MikasaFetchSolver(MikasaPandaArmSapienSolver):
    ...
    #: articulation only while the planning world is built (the sim keeps its +-6.28).
    #: The owner's objection to the recordings (2026-09-09): "огромная намотка углов" —
    #: upperarm_roll to 6.0 rad, 4–9 rad of roll travel per SeasonDish episode, from IK
    #: goals and RRT samples anywhere in +-6.28. Just over one turn: every wrist pose has
    #: an equivalent inside the window (mplib still treats the joint as wrappable, the
    #: window being wider than 2 pi), the +-pi seam of the +-3.141 days (a 331 deg unwind
    #: for a 29 deg move, K54/D14) keeps 0.3 rad of slack, and no plan can wind past it.
    #: 0 = off (the +-6.28 planning space).
    PLAN_ROLL_WINDOW = float(os.environ.get("MIKASA_PLAN_ROLL_WINDOW", "3.44"))
    ...
    @contextlib.contextmanager
    def roll_room(self):
        """The roll joints' planning limits widened to the simulator's own (+-6.28) for the
        plans made inside — for a motion that IS a roll (2026-09-09: SeasonDish's pour is
