...
## `--traj-dir`: the recording, successes only

    python -m utils.mikasa_oracle.evaluate_planner -s MikasaDepthRecall-v1 \
        -p depth_recall_v1_planner -n 50 --obs-mode state --control-freq 10 \
        --control-mode pd_joint_delta_pos --traj-dir raw/seed_1100

Actions plus per-step `get_state_dict()`, no pixels, one `.h5` + `.json` per run with
the episodes keyed `traj_<i>`. **A failed episode is not written** (the owner,
2026-09-10: "Только неуспешные траектории не записываем в данные - просто скипаем"):
the flush is `flush_trajectory(save=verdict == "success")`, which still advances the
buffer pointer, so the file holds successes under contiguous keys and the `.json`
episode list stays in step with them. The failure is still counted in the sweep line,
in the `=== RESULTS ===` summary and in the video clip, so nothing is hidden — only
the trajectory is dropped. `--keep-failed-traj` writes them anyway, for replaying a
failure to see what it did.
...
def parse_args(argv=None):
    ...
    p.add_argument(
        "--keep-failed-traj",
        action="store_true",
        help="write the episodes the oracle lost into --traj-dir too. Off by "
        "default: a VLA dataset is built from successes only, and a half-done "
        "episode teaches the wrong thing. The failure is still counted in the "
        "sweep line, the .json summary and the video, so nothing is hidden — "
        "only the .h5 trajectory is dropped. Turn it on to debug a failure by "
        "replaying its states",
    )
    p.add_argument(
        "--traj-dir",
        default=None,
        ...
        try:
            ...
            if recorder is not None:
                ...
                if args.traj_dir is not None:
                    try:
                        # save=False still advances env_episode_ptr and truncates
                        # the buffer (record.py:717-755) but skips the h5 group,
                        # the _episode_id bump and the .json append — so the file
                        # holds successes under contiguous traj_<i> keys and the
                        # .json episode list stays in lockstep with them.
                        recorder.flush_trajectory(
                            save=args.keep_failed_traj or verdict == "success"
                        )
